...
Vue.component ('settings_bar_podetaps_podetaps', {
    ...
    methods: {
        ...
        close: function() {
            let stages = [],
                mass = [];
            this.podpodetaps.forEach(function (podpodetap){mass.push(podpodetap)})
            console.log(this.podpodetaps)
            this.podetap_of_podetap.povtor.povtor_podetaps.forEach(function (povtor){stages.push(mass[povtor].id)})
            axios.post('/stages/sub/crud/', {
                type: 'updt',
                Stage: JSON.parse(document.getElementById('Stage').textContent),
