new Vue ({
    ...
    created: async function() {
        ...
        for(let i = 0; i < ExeSubStages.length; i++){
            ...
            if (ExeSubStages[i].SubStage == 914){
                    let temp = (await axios.get('/api/execalcparameters/?ExeSubStage='+ExeSubStages[i].id)).data
                    vm.t0 = temp[0].Value
            }

            if (ExeSubStages[i].SubStage == 1036){
                let frac = (await axios.get('/api/execalcparameters/?ExeSubStage='+ExeSubStages[i].id)).data
                console.log(frac)
                for(let i1 = 0; i1 < frac.length; i1++){
                    if (frac[i1].VarName == 'V100'){
                        vm.dolya100 = frac[i1].Value
                    }
                    if (frac[i1].VarName == 'V200'){
                        vm.dolya200 = frac[i1].Value
                    }
                    if (frac[i1].VarName == 'V300'){
                        vm.dolya300 = frac[i1].Value
                    }
                }
                console.log(frac)
            }
        ...
        this.block_height = String(document.documentElement.clientHeight - 298 - count * 116);
        console.log(count)
    }
})
