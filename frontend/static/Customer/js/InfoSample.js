new Vue ({
    ...
    data: {
        ...
        density_value: null, //C БД
        velocity_value: null, //C БД
        sulfur_value: null, //C БД
        fractions_value: 67, //C БД
        asphalt_value: 13, //C БД
    },

    methods: {
        GoInfo: function(stageNumber){
            nextStage = this.infoStage.get((stageNumber+1).toString())
            if (nextStage != null){
                window.location.href = '/customer/'+nextStage+'/?Sample='+this.Sample;
            }
            else{
                window.location.href = '/customer/SubstageTimeSample/?Sample='+this.Sample+'&Number='+(stageNumber+1);
            }
        },
    ...
    created: async function() {
        ...
        for(let i = 0; i < ExeStage.data.length; i++){
            ...
            for(let i = 0; i < ExeSubStages.length; i++){
                ...
                if (ExeSubStages[i].SubStage == 830 || ExeSubStages[i].SubStage == 868){
                    ...
                    vm.density_value = density[0].Value*1000
                }
        ...
        CreateDensityGraph(vm.density_value)
        CreateViscosityGraph(vm.velocity_value)
        CreateSulfurGraph(vm.sulfur_value)
...
// First Graphic
function CreateDensityGraph(density_value){
...
// Second Graphic
function CreateViscosityGraph(viscosity_value){
...
// Third Graphic
function CreateSulfurGraph(sulfur_value){
